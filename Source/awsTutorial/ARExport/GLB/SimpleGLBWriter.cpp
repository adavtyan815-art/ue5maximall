...
bool FSimpleGLBWriter::SerializeToGLB(const TArray<FGLBPrimitive>& Primitives, TArray<uint8>& OutGLBData)
{
    ...
    for (int32 PrimIdx = 0; PrimIdx < Primitives.Num(); ++PrimIdx)
    {
        ...
        // ── 0c. Embed Metallic-Roughness Texture (deduplicated by key) ───────
        int32 MetallicRoughnessTextureIdx = INDEX_NONE;
        if (!Prim.MetallicRoughnessTextureKey.IsEmpty())
        {
            if (const int32* ExistingMRIdx = TextureIndexByKey.Find(Prim.MetallicRoughnessTextureKey))
            {
                MetallicRoughnessTextureIdx = *ExistingMRIdx;
            }
