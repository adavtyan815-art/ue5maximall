...
UTexture2D* FQRCodeTextureHelper::GenerateQRCodeTexture(const FString& TextToEncode, int32 TargetResolution, int32 BorderModules)
{
    ...
    try
    {
        std::string StdString = TCHAR_TO_UTF8(*TextToEncode);
        qrcodegen::QrCode QR = qrcodegen::QrCode::encodeText(StdString.c_str(), qrcodegen::QrCode::Ecc::MEDIUM);
    ...
    {
        UE_LOG(LogTemp, Error, TEXT("[QRCodeTextureHelper] Exception occurred while generating QR Code for '%s'"), *TextToEncode);
        return nullptr;
    }
}
